...
u32 image_length_expected=0;
u8 image_MD5_expected[16]={0};

u32 Firmware_Write_Len=0;
u32 Firmware_Writed_Len=0;
u32 Firmware_Write_Ptr=0;
u32 MD5_status=0;
u8 image_MD5_Calculated[16];
int MD5_Calculated_Length=0;
extern int32_t STM32_MD5_HASH_DigestCompute(uint8_t* InputMessage, uint32_t InputMessageLength,
...
{	
	...
	for(Buffer_index=0;Buffer_index<20;Buffer_index++)
	{
		...
		USART_ITConfig( USART2, USART_IT_TXE,ENABLE ); //Give signal to usart handler to start a 20 bytes' data transfer
	}
		...
		if(L2_RxKeys[0].L2_KEY==KEY_ACCESS_TOKEN) // first key must be access token?
			{
				...
				if(KN_Rcv==RNG_Number) //if kn not right, do nothing
				{
					switch(L2_RxKeys[1].L2_KEY)
					{
						...
						{
							image_length_expected = L2_RxKeys[1].L2_KEY_Value[0]<<16|L2_RxKeys[1].L2_KEY_Value[1]<<8|L2_RxKeys[1].L2_KEY_Value[2];
							memcpy(image_MD5_expected,&L2_RxKeys[1].L2_KEY_Value[3],16);
							bsp_Firmware_Erase(!(*(u8*)(App_ToRun_Flag))); //0 now then erase 1; 1 now then erase 0;
							if(*(u8*)(App_ToRun_Flag))
								Firmware_Write_Ptr = ADDR_FLASH_SECTOR_5;
							else
								Firmware_Write_Ptr = ADDR_FLASH_SECTOR_6; //0 now then ptr 1; 1 now then ptr 0;
						}
						break;
						case KEY_FIRMWARE_WRITEBLOCK:
						{
							Firmware_Write_Len = L2_RxKeys[1].L2_KEY_Header.key_header_bit_field.v_length;
							bsp_Firmware_Write(&L2_RxKeys[1].L2_KEY_Value[0],Firmware_Write_Ptr,Firmware_Write_Len);
							Firmware_Writed_Len += Firmware_Write_Len;
							Firmware_Write_Ptr += Firmware_Write_Len;
							if(Firmware_Writed_Len>=image_length_expected) //enough data
							{
								  L2_TxKeys[0].L2_KEY_Value=0;
									L2_TxKeys[0].L2_KEY=KEY_FIRMWARE_WRITEFINISHED;
									L2_TxKeys[0].L2_KEY_Header.key_header_bit_field.v_length=1;
									L2_TxKeys[0].L2_KEY_Header.key_header_bit_field.reserve=L2_KEY_HEADER_RESERVE;
									L2_TxBuffer[L2_HEADER_CMDID_POS]=COMMAND_FIRMWARE_UPDATE;
									L2_TxKeys_Number=1;
//									while(L1_TxSta!=L1_TX_IDLE);  //wait until L1 tx idle , need  timeout ?????????????????????????
									xQueueSendToBack(L1_TxQue,&WAKEUPSEMA_SEND,portMAX_DELAY);	
								  
								  while(L1_TxSta!=L1_TX_IDLE); //Start MD5 check
								  MD5_status = STM32_MD5_HASH_DigestCompute((uint8_t*)ADDR_FLASH_SECTOR_5,
                                  image_length_expected,
                                  (uint8_t*)image_MD5_Calculated,
                                  &MD5_Calculated_Length);
									if (MD5_status == HASH_SUCCESS)
									{
										if (1)//(Buffercmp(Expected_OutputMessage, MessageDigest, HASH_LENGTH) == PASSED)
										{
									      
											  L2_TxKeys[0].L2_KEY_Value=0;
												L2_TxKeys[0].L2_KEY=KEY_RET_FIRMWARE_CHECK;
												L2_TxKeys[0].L2_KEY_Header.key_header_bit_field.v_length=1;
												L2_TxKeys[0].L2_KEY_Header.key_header_bit_field.reserve=L2_KEY_HEADER_RESERVE;
												L2_TxBuffer[L2_HEADER_CMDID_POS]=COMMAND_FIRMWARE_UPDATE;
												L2_TxKeys_Number=1;
			...
			//									while(L1_TxSta!=L1_TX_IDLE);  //wait until L1 tx idle , need  timeout ?????????????????????????
						...
						break;
						case KEY_START_UPDATE :
						{
							bsp_ChangeAPPtoRun(!(*(u8*)(App_ToRun_Flag))); //0 now then run 1; 1 now then run 0;
							//shut down
						}
						break;
					}
